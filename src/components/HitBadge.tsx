...
const HitBadge: React.FC<HitBadgeProps> = ({ id, label = 'views', color = 'blue', className = '' }) => {
  const url = `${process.env.NEXT_PUBLIC_HIT_COUNTER_URL || 'https://nums-ten.vercel.app'}/badge?id=${encodeURIComponent(id)}&label=${encodeURIComponent(label)}&color=${encodeURIComponent(color)}`;
